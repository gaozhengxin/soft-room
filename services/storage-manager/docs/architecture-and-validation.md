...
| 内网入口 | `http://192.168.2.115:8788` | 当前局域网测试 API |
...
```bash
curl -sS http://192.168.2.115:8788/v1/health
```
...
```bash
curl -sS http://192.168.2.115:8788/v1/config
```
...
测试日期：2026-09-16。测试从另一台局域网 Mac 访问 `192.168.2.115:8788`，不是在容器内部自测。
