...
type Language='zh'|'en';
const copy={
 zh:{title:'选择你的身份',detail:'你可以继续使用临时身份，也可以用已有账户保存加密后的身份和房间。',temporary:'使用临时身份继续',signIn:'登录',create:'创建持久身份',continue:'继续使用 {name}',account:'账户或用户名',password:'App Password',passwordHint:'建议使用账户生成的 App Password。密码不会保存在设备或房间中。',submitSignIn:'登录并恢复',submitCreate:'登录并创建',back:'返回',advanced:'高级设置',pds:'账户服务地址',working:'正在安全地读取身份…',recovery:'这台设备没有该身份的本地主密钥。当前版本尚未提供跨设备恢复密钥导入，请回到原设备继续使用。',missing:'这个账户还没有 Soft Room 持久身份。请选择“创建持久身份”。',exists:'已恢复加密身份和房间。',failed:'暂时无法完成。请检查账户、App Password 和网络。',createHint:'使用已有账户创建 Soft Room 持久身份；不会创建新的社交账户。'},
 en:{title:'Choose your identity',detail:'Continue temporarily, or use an existing account to keep an encrypted identity and room list.',temporary:'Continue with temporary identity',signIn:'Sign in',create:'Create persistent identity',continue:'Continue as {name}',account:'Account or handle',password:'App Password',passwordHint:'Use an App Password generated by your account. It is never saved on this device or sent to a room.',submitSignIn:'Sign in and restore',submitCreate:'Sign in and create',back:'Back',advanced:'Advanced settings',pds:'Account service address',working:'Securely loading your identity…',recovery:'This device does not have the local master key for this identity. Recovery-key import is not available in this version; continue on the original device.',missing:'This account does not have a Soft Room persistent identity. Choose “Create persistent identity”.',exists:'Encrypted identity and rooms restored.',failed:'Could not complete this request. Check the account, App Password and network.',createHint:'Use an existing account to create a Soft Room persistent identity. This does not create a social account.'}
};
...
export async function identityStartup(host:HTMLElement,language:Language):Promise<void>{
 ...
 return new Promise(resolve=>{
  ...
  const showHome=()=>{
   ...
   const create=document.createElement('button');create.textContent=t.create;create.onclick=()=>showForm(true);actions.append(temporary,signIn,create);card.append(actions);
  };
  ...
  const showForm=(create:boolean)=>{
   card.replaceChildren();const back=document.createElement('button');back.className='identity-startup-back';back.textContent='‹ '+t.back;back.onclick=showHome;const h=document.createElement('h1');h.textContent=create?t.create:t.signIn;const hint=document.createElement('p');hint.textContent=create?t.createHint:t.detail;
