...
export class IndexedDbMasterKeys {
 ...
 async deleteProfile(profileId:string){const tx=this.db.transaction(KEYS,'readwrite'),store=tx.objectStore(KEYS);for(const key of [profileId,`recovery-code:${profileId}`,`recovery-record:${profileId}`,`account-session:${profileId}`])store.delete(key);await done(tx);}
}
export class IndexedDbRecordStore implements RecordStore {
 ...
 async deleteAll(){const tx=this.db.transaction(RECORDS,'readwrite'),store=tx.objectStore(RECORDS),keys=store.getAllKeys();keys.onsuccess=()=>{for(const key of keys.result)if(typeof key==='string'&&key.startsWith(`${this.profileId}:`))store.delete(key);};await done(tx);}
