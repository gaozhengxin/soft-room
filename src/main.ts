...
import {activeProfile,deleteSavedRoom,exportRecoveryFile,isPersistent,logoutPersistent,persistSessionState,recoveryPending,refreshPersistentState} from './persistent/runtime.ts';
...
$('attach').onclick=()=>{if(active&&connection?.connected()&&canWrite(active)){$<HTMLFormElement>('file-form').reset();$('file-feedback').textContent='';$('file-wait').hidden=true;showPanel('file-dialog');}else notice('fileNeedRoom');};
