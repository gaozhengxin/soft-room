...
export function challengeFor(publicKey:string,secret:string,now=Date.now(),bits=TURN_BITS):TurnChallenge{
 if(!/^[a-f0-9]{64}$/.test(publicKey)||!Number.isInteger(bits)||bits<1||bits>30)throw Error('Invalid challenge');
...
export function validChallenge(c:TurnChallenge){return !!c&&/^[a-f0-9]{64}$/.test(c.publicKey)&&/^[a-f0-9]{64}$/.test(c.seed)&&Number.isSafeInteger(c.epoch)&&Number.isInteger(c.bits)&&c.bits>=1&&c.bits<=30&&c.expiresAt===(c.epoch+1)*TURN_EPOCH_MS;}
