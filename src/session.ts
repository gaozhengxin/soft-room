...
export type Session={identity:Identity;rooms:SavedRoom[];activeId?:string;language:'zh'|'en';theme?:'soft'|'sssp'};
...
export function encodeSession(s:Session):string{return JSON.stringify({v:1,secret:bytesToHex(s.identity.secret),rooms:s.rooms,activeId:s.activeId,language:s.language,theme:s.theme});}
export function decodeSession(raw:string):Session {
 ...
 return {identity:{secret,publicKey},rooms,...(value.theme==='soft'||value.theme==='sssp'?{theme:value.theme}:{}),language:value.language==='en'?'en':'zh',activeId:rooms.some(x=>roomId(x.room)===value.activeId)?value.activeId:undefined};
}
