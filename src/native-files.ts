...
type NativeFilesPlugin={
 beginSave(options:{name:string;mime:string}):Promise<{cancelled?:boolean;token?:string;error?:string}>;
 writeSaveChunk(options:{token:string;data:string;final:boolean}):Promise<{cancelled?:boolean;error?:string}>;
 openTextFile():Promise<{cancelled?:boolean;text?:string;error?:string}>;
};
...
export async function saveNativeFileDetailed(blob:Blob,name:string):Promise<'unsupported'|'cancelled'|'saved'>{
 if(!nativeFilesAvailable())return 'unsupported';
 ...
 if(!blob.size){const written=await NativeFiles.writeSaveChunk({token:selected.token,data:'',final:true});if(written.error)throw Error(written.error);return written.cancelled?'cancelled':'saved';}
 for(let offset=0;offset<blob.size;offset+=CHUNK_BYTES){
  ...
  const written=await NativeFiles.writeSaveChunk({token:selected.token,data,final:end===blob.size});
  if(written?.error)throw Error(written.error);
...
export function nativeFilesAvailable(){const platform=Capacitor.getPlatform();return platform==='android'||platform==='ios';}
export async function openNativeTextFile():Promise<{status:'unsupported'|'cancelled'|'selected';text?:string}>{
 if(!nativeFilesAvailable())return {status:'unsupported'};
