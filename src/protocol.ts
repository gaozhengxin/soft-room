...
export type Message = { v: 1; room: string; id: string; sender: string; nonce: number; epoch?: number; time: number; text: string; nickname?: string };
...
export function seal(r: Room, identity: Identity, nonce: number, body: string, nickname = '', epoch=dayEpoch()): { message: Message; payload: Uint8Array } {
  if (!body.trim() || body.length > 2000) throw Error('消息需为 1–2000 个字符。');
  const message: Message = {v:1,room:roomId(r),id:bytesToHex(randomBytes(16)),sender:identity.publicKey,nonce,time:Date.now(),text:body.trim()};
...
export function open(r: Room, payload: Uint8Array, now = Date.now()): Message {
  ...
  if (m.v !== 1 || m.room !== roomId(r) || !/^[a-f0-9]{32}$/.test(m.id) || !/^[a-f0-9]{64}$/.test(m.sender) || typeof m.text !== 'string' || !m.text.trim() || m.text.length > 2000 || !Number.isSafeInteger(m.time) || Math.abs(now-m.time) > 300000) throw Error('Invalid message');
