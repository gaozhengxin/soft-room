...
import './mesh.css';
import {channelMode,type ChannelMode,type Network} from './mesh-wire.ts';
...
type Options={host:Element;button:HTMLButtonElement;t:(key:TextKey,params?:Record<string,string|number>)=>string;getMesh:()=>RoomMesh|undefined;canJoin:()=>boolean;name:(key:string)=>string;selfName:()=>string};
...
export function mountMeshPanel(o:Options){
 ...
 const media=new ChannelMedia(()=>view==='channel'?o.getMesh():undefined,()=>render());
 ...
 function finish(showList=false){
  const was=view;view='none';media.stop();if(was==='channel')o.getMesh()?.leave();
 ...
 function reset(){if(view!=='none'){finish();if(history.state?.softRoomChannel===token)history.back();}dialog.close();}
 ...
 function enter(network:Network){
  if(!o.canJoin())return;
  show('channel');try{o.getMesh()!.join(network);render();}catch{finish(true);feedback.textContent=o.t('meshFailed');}
 }
 ...
 $<HTMLFormElement>('channel-create').onsubmit=e=>{
  ...
  try{const mode=page.querySelector<HTMLInputElement>('input[name=mode]:checked')!.value as ChannelMode;view='channel';o.getMesh()!.create($<HTMLInputElement>('channel-name').value.trim()||o.t('meshDefault'),mode);render();$('channel-back').focus();}catch{view='create';error='meshFailed';render();}
 };
 ...
 function render(){
  ...
  if(snapshot!==listSnapshot){listSnapshot=snapshot;list.replaceChildren();if(!groups.length)list.append(el('p',o.t('meshEmpty')));
   for(const group of groups){const card=document.createElement('button');card.type='button';card.className='channel-card';card.dataset.network=group.network.id;card.disabled=!o.canJoin()||!mesh?.supported;card.innerHTML=modeIcon(channelMode(group.network));const detail=document.createElement('span');detail.append(el('strong',group.network.name),el('small',o.t(modeKey(channelMode(group.network)))+' · '+o.t('meshCount',{count:group.people.length})),el('small',o.t('meshCreator',{name:o.name(group.network.creator)})));card.append(detail);card.onclick=()=>enter(group.network);list.append(card);}
  }
  if(view==='none')return;
