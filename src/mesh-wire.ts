...
export type Network={v:1|2;mode?:ChannelMode;id:string;room:string;creator:string;name:string;signature:string};
...
const commitment=(n:Network)=>new TextEncoder().encode(JSON.stringify(n.v===1?['soft-room/mesh/v1',n.id,n.room,n.creator,n.name]:['soft-room/channel/v2',n.id,n.room,n.creator,n.name,n.mode]));
export function createNetwork(room:string,identity:{secret:Uint8Array;publicKey:string},name:string,mode:ChannelMode='voice'):Network{
 ...
 const n:Network={v:2,mode,id:randomId(),room,creator:identity.publicKey,name,signature:''};n.signature=bytesToHex(ed25519.sign(commitment(n),identity.secret));return n;
}
export function validMembership(value:unknown,room:string):value is Membership {
 try{const m=value as Membership,n=m.network;return id(m.instance)&&(n.v===1?n.mode===undefined:n.v===2&&['voice','video','walkie'].includes(n.mode!))&&id(n.id)&&n.room===room&&key(n.creator)&&typeof n.name==='string'&&n.name.length>0&&n.name.length<=32&&n.name.trim()===n.name&&!/[\u0000-\u001f\u007f]/.test(n.name)&&/^[a-f0-9]{128}$/.test(n.signature)&&ed25519.verify(hexToBytes(n.signature),commitment(n),hexToBytes(n.creator));}catch{return false;}
}
