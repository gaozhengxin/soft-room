...
export async function connect(room:Room,receive:(payload:Uint8Array)=>void,signal?:AbortSignal){
 ...
 try{
  ...
  node=await createLightNode({defaultBootstrap:bootstrap.defaultBootstrap,bootstrapPeers:bootstrap.peers,numPeersToUse:privateMode?1:3,connectionManager:{maxBootstrapPeers:bootstrap.peers.length,maxConnections:16},libp2p:{hideWebSocketInfo:true}});
  ...
  const peers=new Map<string,{id:ReturnType<typeof current.libp2p.getPeers>[number];connection:string;lastAttempt:number;pending:boolean;failures:number}>();
  let scanning=false;
  async function scan(){
   ...
   try{
    ...
    await Promise.all(current.libp2p.getConnections().filter(c=>c.status==='open').map(async connection=>{
     ...
     let entry=peers.get(key);if(!entry||entry.connection!==connection.id){entry={id:connection.remotePeer,connection:connection.id,lastAttempt:0,pending:false,failures:0};peers.set(key,entry);health.drop(key);}
     if(entry.pending||Date.now()-entry.lastAttempt<20000)return;
     entry.pending=true;entry.lastAttempt=Date.now();const selected=entry;
     const subscribed=health.available(connections).includes(key);
     const request=subscribed?filter!.ping(entry.id):filter!.subscribe(decoder.pubsubTopic,entry.id,[decoder.contentTopic]);
     void bounded(request).then(result=>{
      if(stopped||peers.get(key)!==selected||live().get(key)!==selected.connection)return;
      if(result.success){selected.failures=0;health.acknowledge(key,selected.connection);return;}
      selected.failures++;if(!subscribed||selected.failures>=3)retire(key,selected.connection);
     }).catch(()=>{selected.failures++;if(peers.get(key)===selected&&(!subscribed||selected.failures>=3))retire(key,selected.connection);}).finally(()=>{selected.pending=false;});
    }));
