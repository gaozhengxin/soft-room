...
export type TurnAccessState={status:'idle'|'requesting'|'mining'|'ready'|'error';elapsed:number};
   ...
   try{
    ...
    while(!signal.aborted){
     if(!proof||proof.challenge.epoch!==turnEpoch(now())){
      ...
      update('requesting');
      const response=await request(challengeURL.href,{signal:AbortSignal.any([signal,AbortSignal.timeout(15000)]),credentials:'omit',cache:'no-store',redirect:'error'});
     ...
     update('requesting');
     const response=await request(url.href,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(proof),signal:AbortSignal.any([signal,AbortSignal.timeout(15000)]),credentials:'omit',referrerPolicy:'no-referrer',cache:'no-store',redirect:'error'});
     if(!response.ok){if(response.status===403){proof=undefined;try{storage?.removeItem('soft-room/turn-proof');}catch{}if(!renewed){renewed=true;continue;}throw Error('Proof rejected');}if(proof!.challenge.epoch!==turnEpoch(now())){proof=undefined;continue;}throw Error('TURN unavailable');}
