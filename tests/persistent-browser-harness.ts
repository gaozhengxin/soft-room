...
declare global {var persistentStorageCheck:()=>Promise<{sameIdentity:boolean;roomKey:string;extractable:boolean}>;}
globalThis.persistentStorageCheck=async()=>{
 ...
 return {sameIdentity:(await repository.loadIdentity())?.publicKey===identity.publicKey,roomKey:(await repository.loadRooms())[0].room.key,extractable};
};
