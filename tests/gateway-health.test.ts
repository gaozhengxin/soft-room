...
test('one acknowledged gateway suffices; losing a spare does not mark the room offline',()=>{
 ...
 assert.deepEqual(health.available(new Map([['a','a2']]),1),[]);assert.deepEqual(health.available(new Map([['a','a1']]),90000),[]);
 health.acknowledge('a','a2',90000);health.drop('a','a1');assert.deepEqual(health.available(new Map([['a','a2']]),90001),['a']);health.clear();assert.deepEqual(health.available(new Map([['a','a2']]),90001),[]);
});
