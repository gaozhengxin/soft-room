...
import {defaultIceServers} from '../src/ice.ts';
...
test('an explicit RTC configuration remains available for direct test harnesses',()=>{
 ...
 const direct={iceServers:defaultIceServers,bundlePolicy:'max-bundle'} as RTCConfiguration;
 const mesh=new RoomMesh({room:roomId(room),identity:local,send:async()=>{},announce:()=>{},changed:()=>{},now:()=>now,rtcConfiguration:direct,peerConnection:config=>{configuration=config;throw Error('Connection unavailable');}});
 mesh.join(network);
...
test('custom TURN remains local, cancels the built-in path, and forces relay',async()=>{
 ...
 assert.ok(calls<=1);assert.equal(config?.iceTransportPolicy,'relay');assert.deepEqual(config?.iceServers?.map(s=>s.urls),['turns:relay.example:443']);assert.ok(!JSON.stringify(mesh.membership).includes('private-password'));assert.ok(!JSON.stringify(mesh.membership).includes('relay.example'));mesh.stop();
});

test('joining a channel requests TURN immediately and creates no peer before relay credentials arrive',async()=>{
 let now=Date.now(),calls=0,resolveIce:(value:RTCConfiguration)=>void=()=>{},config:RTCConfiguration|undefined;
 const [local,remote]=[a,b].sort((x,y)=>x.publicKey.localeCompare(y.publicKey));
 const ice=new Promise<RTCConfiguration>(resolve=>{resolveIce=resolve;});
 const mesh=new RoomMesh({room:roomId(room),identity:local,send:async()=>{},announce:()=>{},changed:()=>{},now:()=>now,iceProvider:async()=>{calls++;return ice;},peerConnection:value=>{config=value;throw Error('Offline');}});
 mesh.join(network);
 const heartbeat=()=>mesh.receive({...seal(room,remote,0,'','',undefined,'heartbeat',membership).message,time:now});
 heartbeat();await Promise.resolve();assert.equal(calls,1);assert.equal(config,undefined);
 resolveIce({iceServers:[{urls:'turns:turn.cloudflare.com:5349?transport=tcp',username:'u',credential:'p'}]});await ice;await new Promise(resolve=>setImmediate(resolve));now++;heartbeat();
 assert.equal((config as RTCConfiguration|undefined)?.iceTransportPolicy,'relay');mesh.stop();
});
