...
try{
 ...
 const image=await page.evaluate(async()=>{const {shareFile}=await import('/media-harness.js'),canvas=document.createElement('canvas');canvas.width=1200;canvas.height=800;const context=canvas.getContext('2d'),data=context.createImageData(canvas.width,canvas.height);for(let offset=0;offset<data.data.length;offset+=65536)crypto.getRandomValues(data.data.subarray(offset,Math.min(data.data.length,offset+65536)));context.putImageData(data,0,0);const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/png')),file=new File([blob],'webkit-fallback.png',{type:'image/png'});Object.defineProperty(globalThis,'createImageBitmap',{value:undefined,configurable:true});const room={v:1,key:'22'.repeat(32),name:'Image check',pow:0};return{source:blob.size,attachment:await shareFile(room,file,'compact')};});
 assert.equal(image.attachment.media,'image');assert.equal(image.attachment.quality,'compact');assert.ok(image.attachment.preview);assert.equal(image.attachment.preview.mime,'image/webp');assert.ok(image.attachment.preview.size<image.source);console.log('PASS WebKit image decode fallback',image.source,image.attachment.preview.size);
