...
test('file references are encrypted, signed, bounded and available in history',()=>{
 const r=makeRoom('files',false),i=makeIdentity(),file:Attachment={v:1,name:'报告.pdf',mime:'application/pdf',bytes:1234,media:'pdf',quality:'original',original:{id:'z'.repeat(40),size:1275,storage:'logos',cipherSha256:'b'.repeat(64)}};
 ...
 assert.throws(()=>seal(r,i,0,'','',undefined,'file',undefined,undefined,{...file,original:{...file.original,cipherSha256:undefined}} as unknown as Attachment));
 assert.throws(()=>seal(r,i,0,'','',undefined,'file',undefined,undefined,{...file,original:{id:'a'.repeat(64),size:1275}} as unknown as Attachment));
