...
try{
 ...
 await page.route('**/turn-negotiation',r=>r.fulfill({contentType:'text/html',body:'<!doctype html><title>Independent TURN negotiation</title>'}));
 ...
 await page.goto('https://127.0.0.1:5173/turn-negotiation');
 await page.evaluate(async({main,delay,tlsOnly,stagger})=>{
  ...
  ids.forEach((identity,i)=>{
   ...
   const provider=createIceProvider('https://soft-room-turn.zhengxingao.workers.dev/ice',async(...args)=>{
 ...
 },{main,delay:Number(process.env.TURN_RESPONDER_DELAY||0),tlsOnly:process.env.TURN_TLS_ONLY==='1',stagger:process.env.TURN_STAGGER!=='0'});
