...
try{
 ...
 for(let i=0;i<2;i++){
  ...
  page.on('response',r=>{if(r.url().includes('.workers.dev/'))console.log('TURN_HTTP '+i+' '+new URL(r.url()).pathname+' '+r.status());});pages.push(page);
 }
 const [a,b]=pages,ready=p=>p.waitForFunction(()=>!document.querySelector('#send')?.disabled,null,{timeout:90000});
 await a.goto(origin,{waitUntil:'domcontentloaded'});await a.locator('#new-room').click();await a.locator('#room-name').fill('Relay verification '+Date.now());await a.locator('.toggle').click();await a.locator('#create-button').click();await ready(a);
 ...
 await b.goto(invite,{waitUntil:'domcontentloaded'});await b.locator('#join-button').click();await ready(b);console.log('WAKU_ROOMS_READY');
