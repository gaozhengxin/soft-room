...
Version 0.2.0 adds encrypted room file sharing backed by the Soft Room Storage service. Images, video, audio, PDF and Markdown can be viewed in a room; every attachment keeps a prominent original-file download. Images support compact previews, supported browsers can create lower-bitrate audio/video previews, and unsupported mobile engines safely keep the original. This build also includes the current Waku history fallback, WebRTC channels, Cloudflare TURN configuration and browser/Android/iOS attachment compatibility paths.
